...
export default function Product() {
  ...
  useEffect(() => {
    ...
    setCategory(navbarItems.find((item) => item.id == selectedProduct?.category_id));
  }, [id]);

  return (
    <div className="min-h-screen w-full p-5 md:px-28">
      <h1 className="text-white text-4xl my-5 font-bold">{category?.title}</h1>
      <div className="flex flex-col justify-center items-center gap-5 lg:items-stretch lg:flex-row lg:gap-0">
        <div className="w-full lg:w-[50%] lg:max-h-96 mr-4 flex justify-center flex-col gap-3">
          <Carousel
            ...
            children={product?.imgs?.map((item, index) => (
              <img
                key={index}
                src={item}
                alt={`HotDog ${index}`}
        ...
        <Card
          className="w-full h-[60%] bg-black border border-gray-400 lg:w-[50%] lg:ml-5 p-5"
          children={
            <>
              ...
              <Tabs value="desc">
                <TabsHeader
                  children={product?.data?.map(({ label, value }: any) => (
                    <Tab
                      value={value}
                      children={<>{label}</>}
                      placeholder={undefined}
                ...
                />
                <TabsBody
                  children={product?.data?.map(({ value, desc }: any) => (
                    <TabPanel
                      className="text-white p-0 py-5"
                      key={value}
                      value={value}
                    >
                      {desc}
              ...
              <div className="w-80">
                ...
                <div className="flex items-center justify-between mt-5 gap-5">
                  <a
                    href={`/product/${id}`}
                    className="w-[50%] flex justify-between items-center px-3 py-2 rounded-lg bg-black hover:bg-gray-800 border border-gray-500"
                  >
